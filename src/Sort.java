...
public class Sort {
    public static int recursionCounter;  // tracks how many recursive calls are made
    private static int mergeCounter;  // tracks how many merges are made

    // merge sort method
    // this method modifies the original elements array object!
    public static void mergeSort(int[] elements) {
        recursionCounter = 0;  // tracks recursive calls for the purpose of printing it below
        int n = elements.length;
        int[] temp = new int[n];
        mergeSortHelper(elements, 0, n - 1, temp);
        System.out.println("Merge sort complete, number of recursive calls: " + recursionCounter);
    }

    // helper method that gets called recursively
    private static void mergeSortHelper(int[] elements, int from, int to, int[] temp) {
        recursionCounter++;  // each time this method gets called recursively, increment count
        if (from < to) {
    ...
    public static void mergeSortPrintDetails(int[] elements) {
        ...
        System.out.println(Arrays.toString(elements));
        System.out.println("Merge sort complete, number of recursive calls: " + recursionCounter);
        System.out.println("Number of merges: " + mergeCounter);
    }

    // helper method that gets called recursively
    private static void mergeSortHelperPrintDetails(int[] elements, int from, int to, int[] temp) {
        recursionCounter++;
        System.out.print("\n  mergeSortHelper recursive call #" + recursionCounter + ": section [");
        for (int i = from; i < to; i++) {
            System.out.print(elements[i] + ", ");
        }
        System.out.println(elements[to] + "]");
        int localCounter = recursionCounter;
        ...
        System.out.println("\n  mergeSortHelper recursive call #" + localCounter + " COMPLETE");
    }
    ...
    public static void selectionSort(int[] elements) {
        ...
        System.out.println("Selection sort complete, number of iterations: " + count);
    }
    ...
    public static void insertionSort(int[] elements) {
        ...
        System.out.println("Insertion sort complete, number of iterations: " + count);
    }
